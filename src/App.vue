...
<template>
  ...
  <main id="main">
    ...
    <section id="schedule" class="section schedule"><div class="section-heading"><p class="eyebrow">{{ t('06 / 赛程与培训') }}</p><h2>{{ t('赛前培训与比赛安排') }}</h2><p>{{ t('以下为建议相对赛程，T 为比赛开始日。') }}<br><ConferenceText :text="t('现场展示与 GOSIM Shenzhen 2026同期举办，具体赛程待公布。')" /></p><div class="schedule-note"><strong>{{ t('必修两讲 + 选修两讲') }}</strong><p>{{ t('非技术参赛者从使用既有 AppCard 生成应用开始；技术参赛者可继续深入 Rust、工具链与协议。讲义也将成为项目文档。') }}</p></div></div><div class="timeline compact-timeline"><details v-for="item in schedule" :key="item.time"><summary><span class="session-time">{{ item.time }}</span><span>{{ t(item.title) }}</span></summary><div><p class="session-type">{{ t(item.type) }}</p><p>{{ t(item.description) }}</p></div></details></div></section>
