...
const faqs = [
  ...
  { q: '从哪里报名？奖金是多少？', a: '9 月 13 日起，通过本页动态二维码或腾讯问卷报名。初赛、复赛和决赛均在线上完成，拟定 10 月 11 日决赛并公布结果；总排名前 2–3 名优胜团队获邀于 10 月 17 日到 GOSIM 现场展示。总奖金池 ¥40,000（含 token 资源）。' },
]
</script>
...
<template>
  ...
  <main id="main">
    ...
    <section id="awards" class="section awards"><div class="section-heading wide"><p class="eyebrow">{{ t('05 / 奖项与评分') }}</p><h2>{{ t('意图设计与技术突破') }}</h2><p>{{ t('总奖金池 ¥40,000（含 token 资源）。一个总冠军，两条奖金对等的评奖线，具体分配赛前公布。') }}</p></div><div class="awards-grid"><article class="champion"><span>{{ t('GRAND PRIZE / 综合分最高') }}</span><h3>{{ t('总冠军') }}</h3><p>{{ t('可以是鹦鹉螺的设计杰作，也可以是大章鱼的系统级突破。最终看作品的综合表现。') }}</p></article><article><span>INTENT DESIGN</span><h3>{{ t('最佳意图设计奖') }}</h3><p>{{ t('评选需求覆盖、信息设计、事实准确性与使用体验。无论卡片还是软件工厂小程序，都应帮助用户完成明确任务。') }}</p></article><article><span>TECHNICAL BREAKTHROUGH</span><h3>{{ t('最佳技术突破奖') }}</h3><p>{{ t('评选数据能力契约质量，以及组件、工具链、协议作品的深度与可合并性。') }}</p></article><article class="adoption"><span>AFTER THE HACKATHON</span><h3>{{ t('采纳奖 · 赛后颁发') }}</h3><p>{{ t('10/18–10/31 采纳窗口结束后，按实际合入仓库的 PR、应用与规范补发；不改变线上决赛排名。') }}</p></article></div><div class="scoring"><div><h3>{{ t('评分如何构成？') }}</h3><p>{{ t('鹦鹉螺的极致设计同样可以拿满创意与完成度分。技术深度加分，不设技术资格门槛。') }}</p></div><div class="score-text-list"><article><div class="score-row-heading"><span>{{ t('项目反哺') }}</span><strong>40%</strong></div><p>{{ t('可合并 PR、入库应用与能力、合格缺口 issue、采纳规范；客观清单及分值赛前公示。') }}</p></article><article><div class="score-row-heading"><span>{{ t('创意与完成度') }}</span><strong>30%</strong></div><p>{{ t('意图到应用的想象力与 Agent 化程度、实际跑通程度，以及服务作品的能力与平台突破。') }}</p></article><article><div class="score-row-heading"><span>{{ t('演示与答辩') }}</span><strong>20%</strong></div><p>{{ t('线上决赛：3 分钟应用 demo + 2 分钟项目贡献，并预留评委问答。') }}</p></article><article><div class="score-row-heading"><span>{{ t('协作与文档') }}</span><strong>10%</strong></div><p>{{ t('让下一个人接得住的 README / spec。混合组队、使用他队能力或工具，双方加分。') }}</p></article></div></div></section>
