...
const bounties = [
  ...
  { title: '跨仓库 schema 契约测试', description: '为 Octoscript 与 octos-one 的接口一致性提供自动检查。', repo: '双仓协作' },
...
<template>
  ...
  <main id="main">
    <section class="hero" aria-labelledby="event-title">
      ...
      <div class="hero-bottom"><span>OCTOS / OCTOS-ONE / OCTOSCRIPT</span><span>{{ t('单一命题 · 三级阶梯 · 双线评奖') }}</span><a href="#challenge">{{ t('探索赛事 ↓') }}</a></div>
    </section>
    ...
    <section class="section stack"><div class="section-heading"><p class="eyebrow">{{ t('03 / 技术平台') }}</p><h2>octos · octos-one · Octoscript</h2><figure class="terminal-figure"><HeroDetail region="computing" :night="displayedNight" :alt="t('主视觉局部：创作者在复古电脑上输入，旁边的手机展示应用卡片')" /><figcaption>{{ t('PERSONAL COMPUTING / 从意图到应用') }}</figcaption></figure></div><div class="stack-list"><article><span>{{ t('体验层') }}</span><h3>octos-one</h3><p>{{ t('Android 原生应用。把意图变成全屏可交互卡片，真实数据由 helper 获取；模型负责布局与绑定。') }}</p></article><article><span>{{ t('语言层') }}</span><h3>Octoscript</h3><p>{{ t('面向模型生成的受限 UI 语言。L0 先校验、再执行，让表达与授权边界清晰可见。') }}</p></article><article><span>{{ t('内核层') }}</span><h3>octos</h3><p>{{ t('Rust 原生 Agentic OS。承载 agent loop、记忆、工具、沙箱与多 agent 编排，通过协议连接应用。') }}</p></article></div></section>
