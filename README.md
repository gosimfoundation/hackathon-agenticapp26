...
`src/components/ApplicationScenarios.vue` now owns the scenario entry points and the messaging focus. Messaging projects use robrix2 as their host and retain its native conversation UI. Software factory mini apps for [hagency](https://github.com/hagency-org/hagency) (formerly hafleet) are one messaging and collaboration example: a task event becomes an actionable card, the user reviews or authorizes an operation, and the result returns to the source conversation. Other scenarios do not require hagency.

The robrix2 mini app runtime integration is planned; its runtime, capability list and starter kit must be verified before being described as available. Source-project captures demonstrate mechanisms, not a completed robrix2 integration. The official scene guides include concept walkthroughs and do not establish that every service is implemented in the competition environment.
