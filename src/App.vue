...
const bounties = [
  { title: 'L0 Web Playground', description: '输入脚本，实时预览卡片。头号悬赏；若赛前已有 MVP，则转为体验增强。', repo: 'Splash' },
  { title: 'check-ui CLI 与 LSP 接入', description: '把 L0 校验带进命令行与编辑器，让创作反馈更及时。', repo: 'Splash' },
  { title: '更有用的错误诊断', description: '错误码与修复建议，帮助模型理解失败并完成自修复。', repo: 'Splash' },
  { title: '结构化 lint 引擎', description: '从子串匹配走向 OR、禁止与计数规则，提升卡片质量校验。', repo: 'octos-one' },
  { title: '跨仓库 schema 契约测试', description: '为 Splash 与 octos-one 的接口一致性提供自动检查。', repo: '双仓协作' },
  { title: 'L0 parser fuzz', description: '测试解析器边界，提高面对复杂输入时的稳定性。', repo: 'Splash' },
  { title: 'L1 规范撰写', description: '把语言行为写成清晰规范，技术写作者也可认领。', repo: 'Splash' },
...
const schedule = [
  ...
  { time: 'T − 4', type: '技术选修 / 120 MIN', title: '第 3 讲 · 悬赏题导览', description: '理解 Splash 工具链与 octos 协议，逐题了解切入点、交付物和验收标准。' },
  ...
  <main id="main">
    <section class="hero" aria-labelledby="event-title">
      ...
      <div class="hero-stage">
        <div class="hero-copy">
          <p class="eyebrow"><a href="https://shenzhen2026.gosim.org/" target="_blank" rel="noopener noreferrer">GOSIM Shenzhen 2026</a> / AGENT2APP</p>
          <h1 id="event-title">The Task<br>Is the <span>App.</span></h1>
          <div class="hero-details">
            <h2>{{ t('Agent2App 黑客松') }}</h2>
      ...
      <div class="hero-bottom"><span>OCTOS / OCTOS-ONE / SPLASH</span><span>{{ t('单一命题 · 三级阶梯 · 双线评奖') }}</span><a href="#challenge">{{ t('探索赛事 ↓') }}</a></div>
    </section>
    <section id="challenge" class="section challenge"><div class="section-heading"><p class="eyebrow">{{ t('01 / 参赛命题') }}</p><h2>{{ t('意图所至，') }}<br>{{ t('应用而生。') }}</h2><figure class="intent-detail"><HeroDetail region="design" :night="displayedNight" :alt="t('主视觉局部：创作者在绘图屏上绘制应用界面')" /></figure></div><div class="section-body"><p class="lead">{{ t('用一句话，生成一个能用的应用。') }}</p><p>{{ t('面向设计师、产品人、OPC（一人公司）、学生，以及懂 Rust 的开发者。围绕 octos-one，从一个真实意图出发，完成一款能够演示的应用。') }}</p><p>{{ t('可以独立参赛，也可以自由组队。赛前组队集市帮助你找到伙伴，赛中平台技术台提供能力接线支持。') }}</p><div class="rule"><span>{{ t('共同交付') }}</span><strong>{{ t('跑通的演示 + 进入仓库的 spec / 代码') }}</strong><p>{{ t('真机优先，桌面演示兜底。无论深入哪一层，最终都要回到「一句话 → 活的 App」。') }}</p></div></div></section>
    ...
    <section class="section stack"><div class="section-heading"><p class="eyebrow">{{ t('03 / 技术平台') }}</p><h2>octos · octos-one · Splash</h2><figure class="terminal-figure"><HeroDetail region="computing" :night="displayedNight" :alt="t('主视觉局部：创作者在复古电脑上输入，旁边的手机展示应用卡片')" /><figcaption>{{ t('PERSONAL COMPUTING / 从意图到应用') }}</figcaption></figure></div><div class="stack-list"><article><span>{{ t('体验层') }}</span><h3>octos-one</h3><p>{{ t('Android 原生应用。把意图变成全屏可交互卡片，真实数据由 helper 获取；模型负责布局与绑定。') }}</p></article><article><span>{{ t('语言层') }}</span><h3>Splash</h3><p>{{ t('面向模型生成的受限 UI 语言。L0 先校验、再执行，让表达与授权边界清晰可见。') }}</p></article><article><span>{{ t('内核层') }}</span><h3>octos</h3><p>{{ t('Rust 原生 Agentic OS。承载 agent loop、记忆、工具、沙箱与多 agent 编排，通过协议连接应用。') }}</p></article></div></section>
    ...
    <section id="participate" class="section participate"><p class="eyebrow">{{ t('08 / 参赛信息') }}</p><h2>{{ t('Agent2App 黑客松 2026') }}</h2><p><ConferenceText :text="t('现场展示与 GOSIM Shenzhen 2026同期举办。报名入口、比赛具体赛程及奖金金额待公布。')" /></p><a class="button primary" href="#steps">{{ t('先了解参赛阶梯 ↗') }}</a><p class="small">{{ t('Agent2App 是独立赛事，与智能体工厂、智能体巡天黑客松并行。') }}</p></section>
  </main>
  <footer><a href="/">GOSIM / CREATE</a><span>{{ t('Agent2App 黑客松 · 2026') }}</span><a href="/">{{ t('查看全部黑客松 ↗') }}</a></footer>
</template>
